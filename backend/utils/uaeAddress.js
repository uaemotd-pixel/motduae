...
export function isValidEmirate(value) {
  return UAE_EMIRATES.some((e) => e.value === value);
}

export function normalizeEmirate(value) {
  if (!value) return "";
...
export function getEmirateEn(value) {
  return UAE_EMIRATES.find((e) => e.value === value)?.en || "";
}

export function getEmirateAr(value) {
  return UAE_EMIRATES.find((e) => e.value === value)?.ar || "";
}
...
export function formatAddressDisplay(address) {
  const parts = [
    ...
    address.street,
    address.building,
    `${address.city}, ${getEmirateEn(address.emirate)}`,
    address.postalCode,
  ].filter(Boolean);
  return parts.join(", ");
}
