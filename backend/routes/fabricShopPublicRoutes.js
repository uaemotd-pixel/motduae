  ...
  try {
    ...
    if (ownerId) {
      const application = await PartnerApplication.findOne({ ownerId })
        .select(
          "website social logoUrl yearsOperating experienceYears experienceMonths experienceBaselineMonths experienceAnchorAt submittedAt createdAt",
        )
        .lean();
      if (application) {
