...
const enrichShopWithApplication = async (shop) => {
  ...
  const application = await PartnerApplication.findOne({
    ...
    .select(
      "businessName businessNameAr about aboutAr logoUrl location area city phone website social licenceNumber licenceFileUrl yearsOperating experienceYears experienceMonths offering partnerNote requestNumber experienceBaselineMonths experienceAnchorAt submittedAt createdAt",
    )
    .lean();
    ...
    if (Array.isArray(req.body.variants)) {
      for (const variant of req.body.variants) {
        ...
        await Fabric.create({
          ...
          patternAr: variant.patternAr || fabric.patternAr || "",
          season: variant.season || fabric.season || "",
          seasonAr: variant.seasonAr || fabric.seasonAr || "",
...
fabricPortalRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (categoryAr !== undefined) fabric.categoryAr = categoryAr || "";
    if (pattern !== undefined) fabric.pattern = pattern || "";
    if (patternAr !== undefined) fabric.patternAr = patternAr || "";
    if (season !== undefined) fabric.season = season || "";
    if (seasonAr !== undefined) fabric.seasonAr = seasonAr || "";
