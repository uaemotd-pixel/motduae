...
const ViewFabricDetails = ({ product }: FabricDetailsProps) => {
    ...
    return (
        <div ref={containerRef} className="min-h-screen bg-[#FFFDF9] antialiased selection:bg-black/10">
            ...
            <div className="pt-20 pb-8 sm:pb-10 lg:pb-12">
                            ...
                            <div className="absolute bottom-4 left-4 right-4 flex gap-3 overflow-x-auto pb-1">
                                {images.map((img, idx) => (
                                    <button
                                        ...
                                        className={`w-20 h-20 shrink-0 overflow-hidden border-2 transition-all duration-300 ${idx === activeImage ? "border-black shadow-lg scale-105" : "border-white/30 hover:border-white/70"
                                            }`}
                                    >
                        ...
                        >
                            ...
                            <div>
                                ...
                                <div className="flex items-baseline gap-4">
                                    ...
                                    <span className="text-sm text-[#5A5A56] font-mono">per meter</span>
                                </div>
                            </div>

                            {/* Color swatches */}
                            <div className="space-y-3">
                                <span className="text-[10px] tracking-[0.2em] uppercase text-[#5A5A56]">Color</span>
                                <div className="flex gap-3">
                                    {Object.entries(colorMap).map(([colorName, hex]) => (
                                        <button
                                            key={colorName}
                                            className={`w-10 h-10 rounded-full border-2 transition-all duration-300 ${product.color.toLowerCase() === colorName
                            ...
                            <div className="grid grid-cols-2 gap-6 py-6 border-t border-b border-[#E8E8E4]">
                                <div>
                                    <span className="text-[10px] tracking-[0.2em] uppercase text-[#5A5A56]">Material</span>
                                    <p className="text-base text-black mt-1">{product.material}</p>
                                </div>
                                <div>
                                    <span className="text-[10px] tracking-[0.2em] uppercase text-[#5A5A56]">Origin</span>
                                    <p className="text-base text-black mt-1">{product.origin}</p>
                                </div>
                                <div>
                                    <span className="text-[10px] tracking-[0.2em] uppercase text-[#5A5A56]">Weight</span>
                                    <p className="text-base text-black mt-1">{product.weight || "250 gsm"}</p>
                                </div>
                                <div>
                                    <span className="text-[10px] tracking-[0.2em] uppercase text-[#5A5A56]">Width</span>
                                    <p className="text-base text-black mt-1">{product.width || "140 cm"}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="flex items-center border border-[#D1D1C8]">
                                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-4 py-3 hover:bg-[#F2F2F0] transition">
                                    ...
                                    <span className="w-12 text-center text-base font-mono">{quantity}</span>
                                    <button onClick={() => setQuantity(quantity + 1)} className="px-4 py-3 hover:bg-[#F2F2F0] transition">
                            ...
                            <div className="border-t border-[#E8E8E4] pt-6 space-y-4">
                                <Accordion title="Care Instructions">
                                    <div className="flex flex-wrap gap-2">
                                        {(product.careInstructions?.length ? product.careInstructions : ["Dry Clean Only", "Do Not Bleach", "Iron Low Heat"]).map((care, i) => (
                                            <span key={i} className="text-xs text-[#5A5A56] bg-[#F2F2F0] px-3 py-1.5">{care}</span>
                                        ))}
                                    </div>
                                </Accordion>
                                ...
                                <Accordion title="Certifications">
                                    <div className="flex gap-4">
                                        {(product.certifications?.length ? product.certifications : ["OEKO-TEX", "GOTS"]).map((cert, i) => (
                                            <div key={i} className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-mono text-[#5A5A56]">{cert}</span>
                                            </div>
                                        ))}
                                    </div>
                                </Accordion>
                ...
                <div className="lg:hidden max-w-screen-2xl mx-auto px-4 sm:px-6 md:px-8">
                        ...
                        <div className="absolute bottom-4 left-4 right-4 flex gap-2 overflow-x-auto pb-1">
                            {images.map((img, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => setActiveImage(idx)}
                                    className={`w-16 h-16 shrink-0 overflow-hidden border-2 transition ${idx === activeImage ? "border-black" : "border-white/50"
                                        }`}
                                >
                                    <img src={img} alt={`View ${idx + 1}`} className="w-full h-full object-cover" />
                                </button>
                    ...
                    >
                        <div className="flex items-center gap-2 text-[10px] tracking-[0.2em] uppercase text-[#5A5A56]">
                            <Link href="/fabrics" className="hover:text-black">Fabrics</Link>
                            <span>/</span>
                            <span className="text-black">{product.category}</span>
                        </div>
                        <h1 className="font-serif text-3xl sm:text-4xl font-light tracking-tight text-black">{product.name}</h1>
                        <div className="flex items-baseline gap-3">
                            <span className="text-2xl sm:text-3xl font-mono">AED {product.price.toLocaleString()}</span>
                        ...
                        <div className="flex gap-3">
                            {Object.entries(colorMap).map(([colorName, hex]) => (
                                <button
                                    key={colorName}
                                    className={`w-10 h-10 rounded-full border-2 transition ${product.color.toLowerCase() === colorName ? "border-black scale-110" : "border-[#E8E8E4]"
                                        }`}
                                    style={{ backgroundColor: hex }}
                                />
                            ))}
                        </div>
                        <p className="text-base text-[#5A5A56] leading-relaxed">{product.description}</p>
                        <div className="grid grid-cols-2 gap-4 py-4 border-t border-b border-[#E8E8E4] text-sm">
                            <div><span className="text-[10px] uppercase text-[#5A5A56]">Material</span><p className="text-black">{product.material}</p></div>
                            <div><span className="text-[10px] uppercase text-[#5A5A56]">Origin</span><p className="text-black">{product.origin}</p></div>
                            <div><span className="text-[10px] uppercase text-[#5A5A56]">Weight</span><p className="text-black">{product.weight || "250 gsm"}</p></div>
                            <div><span className="text-[10px] uppercase text-[#5A5A56]">Width</span><p className="text-black">{product.width || "140 cm"}</p></div>
                        </div>
                        <div className="flex items-center gap-4">
                            ...
                            <select value={selectedSize} onChange={(e) => setSelectedSize(e.target.value)} className="border border-[#D1D1C8] px-3 py-2 text-sm">
                                <option value="meter">Meter</option>
                                <option value="yard">Yard</option>
                                <option value="sample">Sample (30cm)</option>
                            </select>
                        ...
                        <Accordion title="Care Instructions">
                            <div className="flex flex-wrap gap-2">
                                {(product.careInstructions?.length ? product.careInstructions : ["Dry Clean Only"]).map((c, i) => (
                                    <span key={i} className="text-xs bg-[#F2F2F0] px-3 py-1">{c}</span>
                                ))}
                            </div>
                        </Accordion>
                        <Accordion title="Shipping & Returns">
                            <p className="text-sm text-[#5A5A56]">Free shipping over AED 500. 30-day returns.</p>
                        </Accordion>
                    </motion.div>
                </div>
            </div>
            ...
            <AnimatePresence>
                {showStickyCart && (
                    ...
                    >
                        <div>
                            <p className="text-sm font-medium text-black truncate max-w-45">{product.name}</p>
                            <p className="text-xs text-[#5A5A56]">AED {product.price.toLocaleString()} /m</p>
                        </div>
                        <motion.button
                            whileTap={{ scale: 0.95 }}
                            onClick={handleAddToCart}
                            ...
                            >
                                <Link href={`/fabrics/${item.id}`} className="block space-y-4">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-xs text-[#5A5A56] uppercase tracking-[0.15em]">{item.material}</p>
                                        <h3 className="text-sm font-serif font-light text-black truncate">{item.name}</h3>
                                        <p className="text-sm font-mono text-black">AED {item.price.toLocaleString()}</p>
                                    </div>
                                </Link>
                            </motion.div>
                        ))}
