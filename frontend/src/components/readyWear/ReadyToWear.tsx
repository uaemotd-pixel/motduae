...
export function ReadyToWearSection() {
    ...
    // ---------------------------
    // FETCH PRODUCTS (FIXED USING API CLIENT)
    // ---------------------------
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const data = await api.get<any>(
                    `/api/ready-made?locale=${currentLanguage}`
                );
    ...
    return (
        <section className="bg-(--bg-page) py-12 xs:py-16 sm:py-20 md:py-24 lg:py-(--space-80) border-(--color-border) mb-12 xs:mb-16 sm:mb-20 md:mb-24 lg:mb-(--space-80)">
            <div className="px-4 xs:px-6 sm:px-8 md:px-12 lg:px-(--space-40) w-full mx-auto">
                {/* HEADER with styled link (matching premiumFabrics) */}
                ...
                {/* CAROUSEL (fixed group class + styled arrows) */}
                <div className="relative group/carousel">
                    ...
                    <div className="overflow-hidden" ref={emblaRef}>
                        <div className="flex will-change-transform gap-2 xs:gap-3 sm:gap-4 md:gap-5 lg:gap-6">
                            {products.map((item) => {
                                const title = currentLanguage === "ar" ? item.name : item.name;
                                const desc = currentLanguage === "ar" ? item.description : item.description;
                                const image = item.images?.[0] || "/placeholder.png";
                                console.log("Image URL:", image);  // <-- add this
                                        ...
                                        <Link href={`/${localeParams}/ready-made/${item.slug}`}>
                                            ...
                                            <div className="bg-(--bg-page) border border-(--color-border) overflow-hidden transition-all duration-300 hover:shadow-lg h-full flex flex-col">
                                                ...
                                                <div className="p-3 xs:p-4 sm:p-5 md:p-6 lg:p-(--space-24) flex flex-col grow">
                                                    ...
                                                    {/* SIZE BADGE */}
                                                    <span
                                                        className="border w-fit px-2 bg-(--bg-surface) [font-family:var(--font-body)] text-[11px] xs:text-[10px] sm:text-[11px] md:text-[10px] lg:text-[11px] xl:text-[12px] 2xl:text-[13px] leading-relaxed xs:leading-[1.5] sm:leading-[1.6] text-(--color-grey-muted) line-clamp-2 mb-3 xs:mb-3.5 sm:mb-4 grow font-normal"
                                                    >
                                                        Size: {item.size}
                                                    </span>
                                                    ...
                                                    {/* Shop Now button – same styling as premiumFabrics */}
                                                    <button
                                                        onClick={() => {
                                                            // Optional: navigate to product page or handle action
                                                            console.log("Shop now:", item._id);
                                                        }}
                ...
                {/* DOTS – styled with active state (matching premiumFabrics) */}
