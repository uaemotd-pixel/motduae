...
function StorefrontCutPicker({
  ...
  }, [item.fabric, cutOptions, selections]);

  const totalLength = Number(
    availableCuts
      .reduce(
        (sum, cut) => sum + cut.lengthInMeters * (selections[cut.cutId] || 0),
        0,
      )
      .toFixed(2),
  );
  const totalPrice = availableCuts.reduce(
    (sum, cut) => sum + (cut.price || 0) * (selections[cut.cutId] || 0),
    0,
  );
  const leftover =
    totalLength > designMinLength
      ...
      : 0;
  const isShort = totalLength > 0 && totalLength < designMinLength;
  const requirementMet = totalLength + 0.009 >= designMinLength;

  const getCutDisplayName = (cut: { name: string; nameAr?: string }) =>
    locale === "ar" ? cut.nameAr || cut.name : cut.name;

  const setQty = (cut: CustomOrderSelectedCut, quantity: number) => {
    onSetQuantity(
      item.id,
      {
        _id: cut.cutId,
        value: Number(cut.value) || cut.lengthInMeters,
        unit: (cut.unit === "war" ? "war" : "meter") as "war" | "meter",
      },
      quantity,
    );
  };
    ...
    <div className="space-y-6 pt-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {availableCuts.map((cut) => {
          const qty = selections[cut.cutId] || 0;
          const stock = cut.stock ?? 0;
          const atMax = qty >= stock;
          const canIncrease = !requirementMet && !atMax;
          const selected = qty > 0;

          return (
            <div
              key={cut.cutId}
              className={`p-4 border rounded-sm flex flex-col justify-between transition ${
                selected
                  ? "border-black bg-neutral-900 text-white shadow-sm"
                  : "border-(--color-border) bg-white text-black"
              }`}
            >
              <div>
              ...
              <div className="mt-4 pt-3 border-t border-current/10 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {t("piecesLeft", { count: Math.max(0, stock - qty) })}
                  </span>
                </div>

                <div className="flex items-center justify-between gap-2">
                  <span
                    className={`text-[10px] uppercase tracking-[0.16em] [font-family:var(--font-ui)] ${
                      selected ? "text-neutral-300" : "text-(--color-grey-muted)"
                    }`}
                  >
                    {t("quantityLabel")}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={qty <= 0}
                      onClick={() => setQty(cut, qty - 1)}
                      className={`w-8 h-8 border text-sm font-medium transition hover:cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed ${
                        selected
                    ...
                    <span className="min-w-8 text-center [font-family:var(--font-ui)] text-sm font-semibold tabular-nums">
                      {qty}
                    </span>
                    <button
                      type="button"
      ...
      {(Object.keys(selections).length > 0 || isShort) && (
              ...
              >
                ...
                {usingOwnFabric ? (
                  ...
                  <StorefrontCutPicker
                    ...
                    locale={locale}
                    onSetQuantity={setLineItemCutQuantity}
                  />
                )}
              </div>
            );
