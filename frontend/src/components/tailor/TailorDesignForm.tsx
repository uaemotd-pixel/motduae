...
export default function TailorDesignForm({ designId }: TailorDesignFormProps) {
    ...
    const [formData, setFormData] = useState<TailorDesignFormData>(emptyTailorDesignForm());
    ...
    useEffect(() => {
        if (formData.images.length > previousImageCountRef.current) {
            requestAnimationFrame(() => {
                formActionsRef.current?.scrollIntoView({ behavior: "smooth", block: "end" });
            });
        }
    ...
    const handleChange = (field: FieldKey, value: string | number | boolean | string[]) => {
        setFormData((prev) => {
            const next = { ...prev, [field]: value } as TailorDesignFormData;

            if (field === "name" && !isEditMode && !slugTouched && typeof value === "string") {
    ...
    const validate = (): boolean => {
        ...
        if (!Number.isFinite(formData.estimatedMeters) || formData.estimatedMeters <= 0) {
            errors.estimatedMeters = t("validation.estimatedMetersInvalid");
        }
        if (!Number.isFinite(formData.estimatedDays) || formData.estimatedDays < 1) {
    ...
    if (loadError) {
        return (
            <div className="max-w-3xl border border-red-200 bg-red-50 p-8">
                <p className="[font-family:var(--font-body)] text-[14px] text-red-700 mb-4">{loadError}</p>
                <Link
                    href="/tailor/designs"
            ...
            >
                <section className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <FormField label={t("fields.name")} name="name" required error={fieldErrors.name}>
                            <input
                                id="name"
                                type="text"
                                value={formData.name}
                        ...
                        <FormField label={t("fields.nameAr")} name="nameAr" required error={fieldErrors.nameAr}>
                            <input
                                id="nameAr"
                                type="text"
