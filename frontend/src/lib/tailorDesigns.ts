...
export function designToForm(design: TailorDesignProfile): TailorDesignFormData {
    return {
        name: design.name ?? "",
...
export function toTailorDesignPayload(form: TailorDesignFormData): Record<string, unknown> {
    return {
        name: form.name.trim(),
...
export async function fetchTailorDesigns(): Promise<TailorDesignProfile[]> {
    const response = await api.get<{ success: boolean; items: TailorDesignProfile[] }>(
        "/api/tailor/designs",
    );
    return response.items ?? [];
}

export async function fetchTailorDesign(id: string): Promise<TailorDesignProfile> {
...
export async function createTailorDesign(
    ...
    const response = await api.post<{ success: boolean; item: TailorDesignProfile }>(
        "/api/tailor/designs",
...
export async function updateTailorDesign(
    ...
    const response = await api.put<{ success: boolean; item: TailorDesignProfile }>(
        `/api/tailor/designs/${id}`,
        toTailorDesignPayload(form),
    );
    return response.item;
}
