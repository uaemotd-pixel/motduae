...
export default function ReadyMadeCatalogPage() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const message = (err as ApiError)?.message || (err instanceof Error ? err.message : "Something went wrong");
    ...
    const matchesColorFilter = (productColors: string[] | string | undefined, selectedColors: string[]) => {
    ...
    switch (sortBy) {
        case "price-low":
            filteredProducts.sort((a, b) => (a.finalSellingPriceAED || 0) - (b.finalSellingPriceAED || 0));
            break;
        case "price-high":
            filteredProducts.sort((a, b) => (b.finalSellingPriceAED || 0) - (a.finalSellingPriceAED || 0));
            break;
        case "newest":
        default:
                            ...
                            {filters.colors.includes(c.value) && (
                                <span className="absolute inset-0 flex items-center justify-center">
                                    <svg className="w-3 h-3" viewBox="0 0 10 8" fill="none">
                                        <path d="M1 4l3 3 5-6" stroke={c.value === "ivory" || c.value === "sand" || c.value === "white" ? "#000" : "#fff"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                                    </svg>
                                </span>
                            )}
                        </button>
                        ...
                        <div className="w-full text-left">
                            <div className="mb-4 xs:mb-6">
                                <div className="[font-family:var(--font-ui)] text-[10px] xs:text-[9px] sm:text-[10px] md:text-[9px] lg:text-[10px] xl:text-[11px] uppercase tracking-[0.28em] text-[#7A7A72] flex items-center justify-start gap-2 xs:gap-3">
                                    ...
                                    <span>{isAr ? "اكتشف المجموعة" : "Discover the Collection"}</span>
                                    <span className="block w-6 xs:w-8 h-px bg-[#7A7A72]"></span>
                                </div>
                            </div>
                                ...
                                <>
                                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                        {paginatedProducts.map((product) => {
                                            const { title, description } = getReadyMadeDisplayFields(product, locale);
                                            ...
                                            const tailorName = isAr ? product.tailorNameAr || product.tailorName : product.tailorName;
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="absolute top-4 right-4 z-10">
                                                        <WishlistButton
                                                            item={{
                                                                ...
                                                                slug: product.slug,
                                                                quantity: 1,
                                                                ...(Number.isFinite(product.availableFabricStock)
                                                    ...
                                                    <div className="p-4 flex flex-col grow text-left">
                                                        <Link href={`/ready-made/${product.slug}`} className="block relative overflow-hidden mb-4 aspect-4/5 bg-[#F5F5F0] rounded-lg">
                                                            <img
                                                                src={image}
                                                                alt={title}
                                                        ...
                                                        <Link href={`/ready-made/${product.slug}`} className="block hover:opacity-75 transition-opacity">
                                                        ...
                                                        {tailorName && (
                                                            <p className="[font-family:var(--font-ui)] text-[9px] uppercase tracking-[0.24em] text-[#8A8A80] mb-2 font-normal">
                                                                {isAr ? "الخياط: " : "TAILOR: "}{tailorName.toUpperCase()}
                                                            </p>
                                                        )}
