...
export default function TailorDashboardPage() {
  ...
  const [tailorShopId, setTailorShopId] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchPricingOrders = async () => {
      try {
        ...
        const data = await api.get<{ success: boolean; items: Order[]; tailorShopId?: string }>("/api/tailor/orders");
        setPricingOrders(data.items || []);
        if (data.tailorShopId) {
          setTailorShopId(data.tailorShopId);
        }
      } catch (err) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h3 className="[font-family:var(--font-display)] text-lg text-black flex items-center gap-2">
              ...
              {isAr ? "التسعير والرسوم" : "Pricing & Fees"}
            </h3>
            <p className="[font-family:var(--font-body)] text-xs text-(--color-grey-muted) mt-1">
              {isAr
                ? "مراقبة مستحقات تصميم الخياط ورسوم الخياطة لكل طلب تفصيل."
                : "Monitor design fees and tailoring fee earnings scoped to your shop."}
            </p>
          </div>
            ...
            <table className="w-full border-collapse text-left text-xs text-gray-500">
              <thead className="bg-gray-50/70 text-[9px] uppercase tracking-wider text-gray-400 font-semibold border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-4 py-3">{isAr ? "رسوم التصميم" : "Tailor Fee"}</th>
                  <th className="px-4 py-3">{isAr ? "رسوم الخياطة" : "Tailoring Fee"}</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredPricingOrders.map((order) => {
                  ...
                  const tailorFee = getTailorFee(order);
                  const tailoringFee = getTailoringFee(order);
                  ...
                  return (
                    <tr key={order._id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-black">
                        {formatCurrency(tailorFee, order.pricing.currency)}
                      </td>
                      <td className="px-4 py-3 text-black">
                        {formatCurrency(tailoringFee, order.pricing.currency)}
                      </td>
                      <td className="px-4 py-3">
