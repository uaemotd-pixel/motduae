...
export default function AdminSettingsSeasonsPage() {
  ...
  const fetchSeasons = useCallback(
    async (page = 1, limitOverride?: number, searchOverride?: string) => {
      try {
        ...
        const data = await api.get<SeasonsApiResponse>(
  ...
  const openEditModal = (season: Season) => {
    ...
    setEditNameAr(season.nameAr || "");
    setEditIsActive(season.isActive);
  };
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(err, "Failed to create season"));
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.put(`/api/admin/seasons/${editingSeason._id}`, {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-end">
        <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          <button
            type="button"
            onClick={() => fetchSeasons(currentPage)}
      ...
      {!loading && totalItems > 0 && (
        <div className="text-xs text-gray-400 font-medium tracking-wide uppercase">
          {searchQuery.trim()
            ? `${totalItems} matching season${totalItems === 1 ? "" : "s"}`
            : `${totalItems} season${totalItems === 1 ? "" : "s"} total`}
        </div>
      )}
        ...
        <div className="flex flex-col items-center justify-center text-center py-16">
          ...
          <p className="text-sm text-gray-500 max-w-sm">
            {searchQuery
              ...
              : "Use the form above to add your first season."}
          </p>
        </div>
      ) : (
                ...
                <form onSubmit={handleEditSubmit} className="space-y-5">
                  ...
                  <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    >
                      ...
                      {submittingEdit ? "Saving..." : "Update Season"}
                    </motion.button>
                  </div>
                </form>
              </div>
