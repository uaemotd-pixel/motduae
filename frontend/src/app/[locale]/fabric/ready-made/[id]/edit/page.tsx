...
const COLOR_OPTIONS = [
  ...
  { value: "darkslate" + "blue", en: "Dark Slate Blue", ar: "أزرق أردوازي غامق" },
  { value: "darkslate" + "gray", en: "Dark Slate Gray", ar: "رمادي أردوازي غامق" },
  ...
  { value: "lightskyblue", textContent: "Light Sky Blue", ar: "أزرق سماوي فاتح" },
  { value: "lightslate" + "gray", en: "Light Slate Gray", ar: "رمادي أردوازي فاتح" },
  { value: "lightsteel" + "blue", en: "Light Steel Blue", ar: "أزرق فولاذي فاتح" },
...
export default function EditReadyMadePage() {
  ...
  useEffect(() => {
    const loadDropdownData = async () => {
      try {
        const [fabricsRes, tailorsRes, designsRes] =
          await Promise.all([
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        setLoading(true);
        const found = await api.get<any>(`/api/fabric/ready-made/${id}`);
        if (!found) {
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="animate-pulse">
          ...
          <div className="bg-white rounded-2xl p-6 h-96 border border-gray-100"></div>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Update the ready-made product details in your store inventory
        </p>
      </div>
