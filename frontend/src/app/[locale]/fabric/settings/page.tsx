...
import {
  fetchOwnFabricShop,
  updateFabricShopVisibility,
  type FabricShopProfile,
} from "@/lib/fabricShop";

export default function FabricSettingsPage() {
  const { user } = useAuth();
  ...
  const loadShop = useCallback(async () => {
    setLoadingShop(true);
    try {
      setShop(await fetchOwnFabricShop());
    } catch (err) {
      toast.error(
        getApiErrorMessage(
          err,
  ...
  const handleActivate = async () => {
    setSaving(true);
    try {
      const item = await updateFabricShopVisibility({ isActive: true });
      setShop(item);
  ...
  const confirmDeactivate = async () => {
    setSaving(true);
    try {
      const item = await updateFabricShopVisibility({ isActive: false });
      setShop(item);
  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="flex items-start gap-3 mb-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {isAr
                ? "عند إلغاء التفعيل يختفي متجرك ومنتجاتك من الموقع حتى تعيد تفعيله."
                : "Deactivating hides your shop and products across the site until you reactivate."}
            </p>
          </div>
        </div>
      ...
      <ConfirmationModal
        ...
        message={
          isAr
            ? "سيتم إخفاء متجرك ومنتجاتك حتى تعيد تفعيله يدوياً."
            : "Your shop and products will stay hidden until you reactivate them manually."
        }
